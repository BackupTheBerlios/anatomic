...
#            self.upmeasure.update_rate(x)
    ...
    def startRerequester(self):
        # ANATOMIC MODIFICATIONS START HERE
        if self.response.has_key('planted'): # i.e. an anatomic p2p torrent
            ...
            import os
            if os.path.isfile('strackers.dat') is True: # checks if file exists
                try:
                    f = open('strackers.dat', "r")
                except IOError, e:
                    self.errorfunc("WARNING Cannot open file: "+e.filename)
                else:
                    file = f.readline()
                    f.close()
                    try:
                        ...
                        self.errorfunc("WARNING: local strackers.dat is not valid BEncoded data: Using defaults")
                    else:
                        url = bdata
            else:
                self.errorfunc("WARNING: USING DEFAULT SUPERTRACKER LIST")
            qstring = "?"
            qstring += urlencode({"client" : 1, "pull" : self.infohash})
            self.statusfunc(activity = 'searching for trackers')
            import types
            for x in url:
                stracker = x
                x += qstring
                try:
                   h = urlopen(x)
                   data = h.read()
                except : # that is bad - I can't seem to find the type of timeout
                    pass
                else:
                    try:
                      bdata3 = bdecode(data)      # a big fat mess below
                      if bdata3 != "FALSE":
                          if type(bdata3) == types.ListType:
                              for x in bdata3:
                                  if x not in trackers:
                                      trackers.append(x)
                    except ValueError:
                        pass # not particulary important if the supertracker fails
            if len(trackers) > 1:
                self.statusfunc(activity = 'suitable trackers found')
                trackerlist = [trackers]
            elif len(trackers) == 1:
                self.statusfunc(activity = 'suitable tracker found')
                trackerlist = [trackers]
            else:
                trackerlist = [["http://anatomic.berlios.de/network/error/error2.txt"]]
        elif self.response.has_key('announce-list'):
            trackerlist = self.response['announce-list']
        else:
            trackerlist = [[self.response['announce']]]
        # END OF ANATOMIC MODIFICATIONS
